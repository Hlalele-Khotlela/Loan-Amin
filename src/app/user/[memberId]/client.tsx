  ...
  useEffect(() => {
    async function fetchMember() {
      ...
      const res = await fetch(`/api/user/${member_Id}`, {
        ...
        if(!res.ok){
          console.log("Unauthorized:", await res.json());
          return;
        }
    ...
    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div className="rounded-2xl border p-4">
        ...
        <p className="mt-1 text-xl font-semibold">
          M {member.savings?.reduce((acc: number, s: any) => acc + Number(s.amount), 0)}
        </p>
      </div>
      <div className="rounded-2xl border p-4">
        <p className="text-sm text-muted-foreground">Active Loans</p>
        <p className="mt-1 text-xl font-semibold">{member.loan?.length ?? 0}</p>
      </div>
      <div className="rounded-2xl border p-4">
        <p className="text-sm text-muted-foreground">Outstanding Loan</p>
        <p className="mt-1 text-xl font-semibold">
          M {member.loan?.reduce((acc: number, l: any) => acc + Number(l.balance), 0)}
        </p>
      </div>
      <div className="rounded-2xl border p-4">
        <p className="text-sm text-muted-foreground">Group Savings</p>
        <p className="mt-1 text-xl font-semibold">{member.groupSavings?.length ?? 0} Groups</p>
      </div>
    </div>
  </div>
)}
            ...
            <div>
              ...
              {member.savings?.map((s: any) => (
                <div key={s.savings_id} className="mb-4 border p-4 rounded">
                  <p className="font-medium">Account: {s.savings_type ?? "Unnamed"}</p>
          ...
          {activeTab === "Group Savings" && (
            <div>
              ...
              {member.GroupDeposits?.map((d: any) => (
                <div key={d.id} className="flex justify-between border p-2 rounded mb-2">
                  <span>Deposit</span>
              ...
              {member.GroupWithdrawal?.map((w: any) => (
                <div key={w.id} className="flex justify-between border p-2 rounded mb-2">
        ...
        <div key={idx} className="mb-6 relative">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              {item.loanId && (
                <p className="text-sm text-muted-foreground">Loan #{item.loanId}</p>
              )}
